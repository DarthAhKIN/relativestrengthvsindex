...
if prices_dict:
    # 데이터 통합 및 휴장일 선형 보간 (점선 효과를 위한 처리)
    ...
    # --- 3. 메인 화면 상단 구성 ---
    st.title("📈 주식 & 원자재 수익률 통합 분석")
    
    st.markdown("### 👁️ 분석 대상 선택")
    selected_symbols = st.multiselect(
        "그래프에 표시할 항목을 선택하세요 (선택 해제 시 최고가 마크도 함께 제거됩니다)",
    ...
    if not selected_symbols:
        st.warning("최소 하나 이상의 종목을 상단 선택창에서 선택해주세요.")
    else:
        # 데이터 필터링
        ...
        # --- 5. 하단 분석 리포트 (상관계수 & 성과 요약) ---
        col_left, col_right = st.columns([1, 1])

        with col_left:
            st.subheader("항목 간 상관관계")
            if len(selected_symbols) > 1:
                # 보간된 데이터를 바탕으로 결측치 없이 상관계수 계산
                ...
                st.info("상관관계를 분석하려면 2개 이상의 종목을 선택하세요.")
        ...
        with col_right:
            ...
            for col in filtered_prices.columns:
                ...
                a_vol = daily_rets[col].std() * np.sqrt(252) * 100
            ...
            st.download_button(
                label="📊 분석 결과 CSV 저장",
                data=csv,
                file_name=f'stock_commodity_analysis_{start_date}.csv',
                mime='text/csv',
            )
            st.info("※ **변동성**은 해당 기간의 일일 수익률 표준편차를 기준으로 산출되었습니다.")

else:
    st.error("데이터를 불러올 수 없습니다. 인터넷 연결이나 티커명을 확인해주세요.")
