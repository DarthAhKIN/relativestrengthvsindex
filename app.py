    ...
    for name, sym in symbols.items():
        try:
            ...
            if not df.empty:
                ...
                # 고가, 저가, 종가 데이터 추출 (멀티인덱스 방지)
                temp_df = pd.DataFrame(index=df['Date'])
                for col_name in ['Close', 'High', 'Low']:
                    col_data = df[col_name]
                    temp_df[col_name] = col_data.iloc[:, 0] if isinstance(col_data, pd.DataFrame) else col_data
                prices_dict[name] = temp_df
        except: continue

if prices_dict:
    st.title("📈 주식 & 원자재 통합 분석")
    ...
    if not selected_symbols:
        ...
        for i, col in enumerate(selected_symbols):
            ...
            fig.add_trace(go.Scatter(
                ...
                opacity=0.15, name=f"{col} 변동폭", legendgroup=col, showlegend=False, hoverinfo='skip'
            ), row=1, col=1)

            fig.add_trace(go.Scatter(
                x=norm_close.index, y=norm_close, name=col, legendgroup=col,
            ...
            # --- 에러 수정 포인트: 단일값 추출 ---
            current_min = float(drawdown.min()) 
            all_min_dd.append(current_min)
            
            fig.add_trace(go.Scatter(
                x=drawdown.index, y=drawdown, name=col, legendgroup=col, showlegend=False,
                mode='lines', line=dict(width=1.5, color=color), fill='tozeroy',
        ...
        for col in selected_symbols:
            ...
            summary_list.append({'종목': col, '수익률 (%)': round(ret, 2), 'MDD (%)': round(mdd, 2)})
